...
def minimum_tracking_error(cov_mat, w_lower=0.0, w_upper=1.0, convex=False):
    """
    determine the portfolio allocations w that minimize tracking error
    ...
    Returns:
        (numpy array): the values that produce the minimum tracking error portfolio
    """        
    m, n = cov_mat.shape
    assert m == n
    assert w_lower <= w_upper
    ...
    if convex:
        w = cp.Variable(m)
        objective = cp.Minimize(cp.sqrt(cp.quad_form(w, cov_mat) * np.sum(1 - 2*sigma_bm*rho_p_bm) + sigma_bm**2))
        constraints = [w >= w_lower, w <= w_upper, sum(w) == 1]
        problem = cp.Problem(objective, constraints)
